...
def main() -> None:
    ...
    vis_cmd.add_argument(
        "--format",
        choices=["html", "graphml", "cypher", "obsidian", "svg", "contract"],
        default="html",
    ...
    try:
        if args.command == "build":
            ...
            if fmt == "graphml":
                ...
                from .exports import export_agent_contract

                out = data_dir / "code_review_graph_contract.json"
                export_agent_contract(store, out, repo_root=repo_root)
                print(f"Agent contract exported: {out}")
            else:
