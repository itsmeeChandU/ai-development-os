#!/usr/bin/env python3
"""Generate repository-backed production API service artifacts."""

from __future__ import annotations

import json
import sys
from pathlib import Path


ROOT = Path(__file__).resolve().parent.parent
SRC = ROOT / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

from importer_source_readiness.production_api_service import (
    build_production_api_service,
    write_production_api_service_artifacts,
)


def main() -> int:
    payload = build_production_api_service(ROOT)
    paths = write_production_api_service_artifacts(payload, ROOT)
    print(
        json.dumps(
            {
                "status": payload["status"],
                "simulated_request_count": payload["simulated_request_count"],
                "safe_read_success_count": payload["safe_read_success_count"],
                "tenant_denial_count": payload["tenant_denial_count"],
                "unauthenticated_denial_count": payload["unauthenticated_denial_count"],
                "effect_gate_closed_count": payload["effect_gate_closed_count"],
                "claims_opened": payload["claims_opened"],
                "external_effects_created": payload["external_effects_created"],
                "manifest": str(paths["manifest"].relative_to(ROOT)),
            },
            sort_keys=True,
        )
    )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
