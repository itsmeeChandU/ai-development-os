#!/usr/bin/env python3
"""Generate production evidence claim-gate artifacts."""

from __future__ import annotations

import json
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent
SRC = ROOT / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

from importer_source_readiness.production_evidence_claim_gate_engine import (
    build_production_evidence_claim_gate_engine,
    write_production_evidence_claim_gate_engine_artifacts,
)


def main() -> int:
    payload = build_production_evidence_claim_gate_engine(ROOT)
    paths = write_production_evidence_claim_gate_engine_artifacts(payload, ROOT)
    print(
        json.dumps(
            {
                "status": payload["status"],
                "claim_type_count": payload["claim_type_count"],
                "claim_gate_decision_count": payload["claim_gate_decision_count"],
                "safe_research_claim_count": payload["safe_research_claim_count"],
                "blocked_claim_count": payload["blocked_claim_count"],
                "forbidden_external_claim_count": payload["forbidden_external_claim_count"],
                "evidence_mapper_count": payload["evidence_mapper_count"],
                "claim_gate_mapper_count": payload["claim_gate_mapper_count"],
                "claims_opened": payload["claims_opened"],
                "external_effects_created": payload["external_effects_created"],
                "manifest": str(paths["manifest"].relative_to(ROOT)),
                "decisions": str(paths["decisions"].relative_to(ROOT)),
                "mappers": str(paths["mappers"].relative_to(ROOT)),
                "doc": str(paths["doc"].relative_to(ROOT)),
            },
            sort_keys=True,
        )
    )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
