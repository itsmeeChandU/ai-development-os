...
REQUIREMENTS_TRACEABILITY = [
    ...
    {
        "id": "REQ-ENTERPRISE-01",
        "name": "Enterprise API and workspace contracts",
        "status": "implemented_local_contracts_with_external_gates",
        "artifacts": ["production_enterprise_api_manifest.json", "production_enterprise_api_contracts.json"],
        "proof": ["tests/test_production_enterprise_api_platform.py", "scripts/check_product.py"],
        "boundary": "Enterprise APIs, keys, webhooks, audit exports, usage limits, and white-label report rules are local contracts; hosted auth, live keys, webhook delivery, real customer terms, and security review remain external gates.",
    },
    {
        "id": "REQ-PUBLIC-01",
...
def build_runtime_state(workflow: dict[str, Any], *, extra_audit_events: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "api_routes": [
            ...
            "/api/trade-discovery",
            "/api/trade-data-catalog",
            "/api/opportunities",
            "/api/country-coverage",
            "/api/billing/controls",
            "/api/billing/usage",
            "/api/payments/monetization",
    ...
    artifacts = {
        ...
        "public_trade_readiness_manifest.json": {
            ...
            "modes": {
                ...
                "trade_discovery": "Lets beginner users browse Canada import/export categories and country lanes as source-routed research only.",
                "trade_data_catalog": "Turns discovery choices into source-routed query plans, browse cards, and dataset work orders without showing values.",
                ...
                "market_readiness_evidence_room": "Maps real go-live input requests into operator work orders, captures returned inputs locally, and shows a returned-input ledger and history trail while keeping market-ready claims closed.",
            },
