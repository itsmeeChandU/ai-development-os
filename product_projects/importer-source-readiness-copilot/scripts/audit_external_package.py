...
LOCAL_PATH_EXAMPLE_FILES = {"REDACTION_REPORT.md", "PACKAGE_AUDIT.md"}
TEXT_SUFFIXES = {
    ".csv",
    ".html",
    ".json",
...
def _check_text(name: str, text: str) -> list[str]:
    errors = []
    allow_local_path_examples = Path(name).name in LOCAL_PATH_EXAMPLE_FILES
    if not allow_local_path_examples:
...
def _is_skipped_file(name: str) -> bool:
    ...
    return normalized in SKIP_FILES or any(normalized.endswith(f"/{skipped}") for skipped in SKIP_FILES)
